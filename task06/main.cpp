...
void WdWddW_Spring3(
  ...
  const Eigen::Vector3d x = (node2xyz[0] - node2xyz[1]);
  const Eigen::Matrix3d X = x * x.transpose();
  const Eigen::Matrix3d H = (I * stiffness) - (I * (stiffness * length_ini / length)) + X * (stiffness * length_ini / (length * length * length));
  ddw[0][0] = H;
  ddw[0][1] = -H;
  ddw[1][0] = -H;
  ddw[1][1] = H;
