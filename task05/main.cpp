...
void wdw_spring_3d(
  ...
  dw[0] = stiffness * ((node2xyz[0] - node2xyz[1]) - length_ini * (node2xyz[0] - node2xyz[1]) / length);
  dw[1] = stiffness * ((node2xyz[1] - node2xyz[0]) - length_ini * (node2xyz[1] - node2xyz[0]) / length);
}
...
int main() {
  constexpr float learning_rate = 0.6e-3f;
